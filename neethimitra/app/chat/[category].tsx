...
import AnimatedR, { useSharedValue, useAnimatedStyle, withSpring } from 'react-native-reanimated';
...
function MessageBubble({
  ...
  scale: number;
  t: any;
  playResponseAudio: (uri?: string) => void;
}) {
  ...
  return (
    <View style={[styles.msgRow, isUser ? styles.msgRowUser : styles.msgRowAI]}>
      ...
      <View style={[
        ...
        {!showTranslation ? (
          <Text style={[styles.bubbleText, { color: isUser ? '#FFFFFF' : C.text, fontSize: 14 * scale }]}>
            {item.text}
          </Text>
        ) : (
          <View style={{ gap: 6 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginVertical: 2 }}>
              <View style={{ flex: 1, height: 1, backgroundColor: isUser ? 'rgba(255,255,255,0.25)' : C.surfaceBorder }} />
              <Text style={{ fontSize: 13, color: isUser ? '#FFFFFF' : Colors.orange }}>⇄</Text>
              <View style={{ flex: 1, height: 1, backgroundColor: isUser ? 'rgba(255,255,255,0.25)' : C.surfaceBorder }} />
            </View>
            {/* English translation */}
            <Text style={[styles.bubbleText, { color: isUser ? 'rgba(255,255,255,0.85)' : C.textSecondary, fontSize: 14 * scale, fontStyle: 'italic' }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, gap: 10 }}>
          ...
          {hasTranslation && (
            <TouchableOpacity
              ...
              style={[
                styles.translateBtn,
                {
                  borderColor: isUser ? 'rgba(255,255,255,0.35)' : Colors.orange + '40',
                  backgroundColor: showTranslation
                    ? (isUser ? 'rgba(255,255,255,0.15)' : Colors.orange + '20')
                    : 'transparent',
                }
              ]}
              activeOpacity={0.7}
            >
              <Text style={[styles.translateBtnText, { color: isUser ? '#FFFFFF' : Colors.orange, fontSize: 11 * scale }]}>
          ...
          {!isUser && item.audioUri && (
            ...
            >
              ...
              <Text style={[styles.speakBtnText, { color: Colors.orange, fontSize: 11 * scale }]}>{t.play}</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
...
export default function ChatScreen() {
  ...
  const {
    ...
    textSize, isAnonymousGuest, guestQueriesRemaining, decrementQueries,
  } = useAppStore();
  ...
  const playResponseAudio = async (audioUri?: string) => {
    if (!audioUri) return;
